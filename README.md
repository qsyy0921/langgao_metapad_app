...
当前阶段是模型训练原型、服务器原型和 Android MVP 并行开发阶段。Android 工程已创建并可构建 debug APK；检测接口仍处于服务端联调前状态。
...
- [YOLO 分割配置](model-training/configs/20260615/yolo-seg/toolbox-seg-v1.yaml)
- [PatchCore 配置](model-training/configs/20260615/patchcore/toolbox-patchcore-v1.yaml)
...
- 拍照界面：CameraX 预览、默认尝试开启补光、拍照、重拍、照片确认。
- 检测界面：上传/检测中、原图、检测图占位、PASS/NG、四类检测清单，只保留 `返回拍照`。
- 存储界面：记录列表和同步状态原型。
- 设置界面：自动存储开关、存储位置、服务器地址、`/api/health` 连接测试。
...
- [SDD 设计记录](docs/SDD.md)
- [App 开发规划](android-app/docs/DEVELOPMENT_PLAN.md)
...
1. 已完成：创建 Android APK 工程。
2. 已完成：极简版拍照、检测、存储、设置界面。
3. 已完成：接入 CameraX 拍照和默认补光尝试。
4. 待完成：接入本机服务器 `/api/inspect`。
5. 待完成：展示服务器返回的检测图、PASS/NG 和检测清单。
6. 待完成：使用 Room 保存本地记录。
